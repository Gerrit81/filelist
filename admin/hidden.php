    ...
    if (isset($_POST['add_hidden'])) {
        ...
        if (!empty($newHidden) && !in_array($newHidden, $config['hidden_files'])) {
            ...
            if (saveConfig($config) !== false) {
                ...
                $message = '已添加隐藏规则';
                // 防呆：规则内部含空格（非首尾）很可能是误输入，如 sync.ffs_db 被写成 sync .ffs_db
                if (strpos($newHidden, ' ') !== false) {
                    $messageType = 'warning';
                    $message .= '；⚠ 注意：该规则包含空格，请确认文件名中确实有空格（常见误输入：sync.ffs_db 被写成 sync .ffs_db，导致永远匹配不上）';
                }
            } else {
                ...
                <div style="background: #f8f9fa; border-radius: 8px; padding: 10px;">
                    <?php foreach ($hiddenFiles as $index => $rule): ?>
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; border-bottom: 1px solid #e9ecef;">
                            <span style="font-family: monospace; color: #495057;"><?php echo htmlspecialchars($rule); ?><?php if (strpos($rule, ' ') !== false): ?> <span style="background:#fff3cd; color:#856404; border:1px solid #ffe69c; border-radius:4px; padding:1px 6px; font-family:sans-serif; font-size:12px;" title="规则包含空格，请确认与真实文件名一致，例如 sync.ffs_db 不要写成 sync .ffs_db">⚠ 含空格</span><?php endif; ?></span>
