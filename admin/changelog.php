...
$changelog = array(
    array(
        ...
        'date'    => '2026-08-18',
        'changes' => array(
            '🛡️ 后台隐藏文件管理新增防呆提示：规则含空格时添加后黄色警告，列表中标注「⚠ 含空格」，防止误输入导致规则永不匹配',
        ),
    ),
    array(
        'version' => '2.11.2',
        'date'    => '2026-08-18',
        'changes' => array(
            '🎯 返回顶部按钮位置再次右移两个中文字符（相对列表右缘共约四个中文字符，标准/窄版布局同步适配）',
        ),
    ),
    array(
        'version' => '2.11.1',
        'date'    => '2026-08-18',
        'changes' => array(
            '🎯 返回顶部按钮位置微调：相对文件列表右缘再向右移动约两个中文字符（标准/窄版布局同步适配）',
        ),
    ),
    array(
        'version' => '2.11.0',
        'date'    => '2026-08-18',
        'changes' => array(
            '⬆️ 前台新增「返回顶部」悬浮按钮：滚动超过一定距离后自动出现，点击平滑返回顶部；SVG 箭头图标随主题渐变配色，位置紧贴文件列表右侧边缘（自动适配标准/窄版布局）',
        ),
    ),
    array(
        'version' => '2.10.1',
        'date'    => '2026-08-18',
        'changes' => array(
            '🐛 修复 Windows 下隐藏文件不生效：config.json 若被记事本等保存为带 UTF-8 BOM 会导致解析失败、配置整体回退默认值，现自动剥离 BOM 兼容',
            '🐛 隐藏规则在 Windows 下改为大小写不敏感匹配：desktop.ini 可匹配 Desktop.ini（Thumbs.db / Desktop.ini 等系统文件也能隐藏）',
            '🐛 修复通配符隐藏规则中正则特殊字符被误解释的问题',
            '🛡️ 配置保存失败不再静默：后台隐藏文件管理、系统设置、导航链接编辑在无法写入 config.json 时明确报错提示',
        ),
    ),
    array(
        'version' => '2.10.0',
        'date'    => '2026-08-18',
        'changes' => array(
            '📦 运行期数据与程序代码分离：config.json / app.db / session / cache / thumbs 统一存放于「运行数据目录」（默认 runtime/，可在系统设置中改为项目外路径），升级时保留该目录即不丢任何配置与数据',
            '🔁 自动迁移旧版数据：升级后首次访问自动把根目录散落的运行期数据收拢到 runtime/',
            '🚚 后台修改运行数据目录后自动迁移数据到新位置',
            '🏷️ 版本号改为单一来源（config.php），修复前台版本号与后台更新日志不一致的问题',
        ),
    ),
    array(
        'version' => '2.9.8',
        'date'    => '2026-08-18',
        'changes' => array(
            '🔧 数据目录验证新增逐级路径诊断：定位断链目录、检测权限不足层级、列出卷下实际共享文件夹名称',
            '💡 补充群晖 Web Station 高级共享权限排查提示与 open_basedir 选项位置说明',
        ),
    ),
    array(
        'version' => '2.9.7',
        'date'    => '2026-08-18',
        'changes' => array(
            '🐛 修复群晖 WebStation 下数据目录误报「目录不存在或无法读取」：open_basedir 限制改为路径前缀解析判断（不依赖 shell_exec），报错更精准并补充 http 用户权限排查指引',
        ),
    ),
    array(
        'version' => '2.9.6',
