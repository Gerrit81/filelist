...
/**
 * 判断目标路径是否在 open_basedir 允许范围内（纯字符串前缀匹配，不依赖 shell 命令）
 * 用于群晖 WebStation 等默认禁用 shell_exec 的环境下，准确诊断 open_basedir 拦截
 */
function pathAllowedByBasedir($path, $basedir) {
    if (empty($basedir) || !is_string($basedir)) {
        return true; // 未启用 open_basedir → 不限制
    }
    ...
    foreach (explode($sep, $basedir) as $allowed) {
        $dir = rtrim(str_replace('\\', '/', trim($allowed)), '/');
        if ($dir === '' || $dir === '.') {
            continue;
        }
        // open_basedir 按"目录子树"匹配：/volume1/web 允许 /volume1/web 及其中内容，但不允许 /volume1/web2
        if ($target === $dir || strpos($target, $dir . '/') === 0) {
            return true;
        }
    }
    return false;
...
/**
 * 逐级诊断目录为何对 PHP 不可见（不依赖 shell，群晖禁用 shell_exec 也能用）
 * 返回诊断详情数组（元素为已含 <code> 标签的文本，调用方直接 implode('<br>') 输出）
 */
function diagnosePathIssue($path) {
    $details = array();
    ...
    $parts = array_values(array_filter(explode('/', ltrim($path, '/')), 'strlen'));

    // 逐级检查：找到断链点 / 无权限点
    $current = '';
    $lastExisting = null;
    $missingLevel = null;
    $unreadableLevel = null;
    foreach ($parts as $i => $seg) {
        $current .= '/' . $seg;
        clearstatcache(true);
        if (@is_dir($current)) {
            $lastExisting = $current;
            if ($unreadableLevel === null && !@is_readable($current)) {
                $unreadableLevel = $current;
            }
        } else {
            $missingLevel = $current;
            break;
        }
    }
    ...
    if ($missingLevel !== null) {
        if ($lastExisting !== null) {
            $details[] = 'PHP 可正常访问到 <code>' . htmlspecialchars($lastExisting) . '</code>，但其下级 <code>' . htmlspecialchars($missingLevel) . '</code> <strong>不存在</strong>。请核对后半段路径的拼写、大小写与层级（例如共享文件夹名、子目录名是否写错）。';
        } else {
            $details[] = '路径第一级 <code>' . htmlspecialchars($missingLevel) . '</code> 就检测不到。';
        }
    }

    if ($unreadableLevel !== null) {
        $details[] = '检测到 <code>' . htmlspecialchars($unreadableLevel) . '</code> <strong>存在但 PHP 无读取权限</strong>（Permission denied）。请到「控制面板 → 共享文件夹 → 权限」为 <code>http</code> 用户勾选「可读写」；若已授权请检查该目录下是否嵌套了权限受限的子文件夹。';
    }

    // 比对卷下实际共享文件夹名（帮助发现名称/大小写错误）
    if ($parts && preg_match('#^(/volume\d+)/#', $path, $m)) {
        $vol = $m[1];
        $share = isset($parts[1]) ? $parts[1] : '';
        clearstatcache(true);
        $list = @scandir($vol);
        if (is_array($list)) {
            $realShares = array();
            ...
            if ($share !== '' && !in_array($share, $realShares, true)) {
                $details[] = '在 <code>' . htmlspecialchars($vol) . '</code> 下未找到名为 <code>' . htmlspecialchars($share) . '</code> 的共享文件夹。该卷实际存在的共享文件夹：<code>' . htmlspecialchars(implode('</code>、<code>', $realShares)) . '</code>。请对照检查名称与大小写。';
            }
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'reorder_nav') {
        ...
        if (isset($saved) && $saved === false) {
            echo json_encode(['ok' => false, 'error' => '保存失败：无法写入配置文件 config.json，请检查运行数据目录的写入权限']);
        } else {
    ...
    if (isset($_POST['data_dir'])) {
        ...
        if (!$exists) {
            ...
            if ($rp !== false && is_dir($rp)) {
            ...
            } elseif (!empty($basedir) && !pathAllowedByBasedir($newDataDir, $basedir)) {
                // 目标路径不在 open_basedir 允许范围内 → 必然被拦截
                // （纯字符串解析判断，群晖等禁用 shell_exec 的环境下也能准确识别）
                $message = '数据目录无效：<strong>目录被 PHP open_basedir 限制无法访问。</strong>'
                    . '<br><br>当前 open_basedir：<code>' . htmlspecialchars($basedir) . '</code>'
                    . '<br><br>目标目录 <code>' . htmlspecialchars($newDataDir) . '</code> 不在上述允许范围内。'
                    . '<br><br><strong>解决方法（选其一）：</strong>'
                    . '<br>① 群晖 Web Station → 脚本语言设置 → 选择 PHP 8.0 配置文件 → 编辑 → 「设置」选项卡（部分版本在「核心设置」/「常规设置」）→ 勾选「自定义 PHP open_basedir」并追加 <code>:' . htmlspecialchars($newDataDir) . '</code>（多个路径用冒号分隔）。若你的界面根本没有 open_basedir 选项，说明该 PHP 配置未启用 open_basedir 限制，可在「自定义 php.ini」选项卡中手动添加 <code>open_basedir = ' . htmlspecialchars($newDataDir) . '</code>'
                ...
                // 目录确实不存在，或 PHP 进程无权限读取
                // 先做逐级路径诊断（不依赖 shell），再尝试 shell 二次确认
                $diagDetails = diagnosePathIssue($newDataDir);
                if (empty($diagDetails)) {
                    $diagDetails = array('PHP 无法访问该路径的任何一级目录。请确认：');
                    $diagDetails[] = '① 路径填写是否正确（可在 File Station 中右键目标目录 → 属性，查看其真实路径）；';
                    $diagDetails[] = '② 是否在「控制面板 → 共享文件夹 → 权限」中为 <code>http</code> 用户开放了「可读写」；';
                    $diagDetails[] = '③ 是否在「共享文件夹 → 编辑 → 高级权限」中启用了「对所有应用程序（如 Web Station）授予访问权限」。';
                }
                ...
                if ($dirOnDisk) {
                    // 目录确实存在，但 PHP 进程（群晖下为 http 用户）无法读取 → 权限问题
                    $message = '数据目录无效：<strong>目录在服务器上存在，但 PHP 进程无法读取。</strong>'
                        . '<br><br>请检查目录权限：'
                        . '<br>① 群晖：控制面板 → 共享文件夹 → 权限 → 为 <code>http</code> 用户勾选「可读写」'
                        . '<br>② 群晖：共享文件夹 → 编辑 → 高级权限 → 勾选「对所有应用程序授予访问权限」（重要，Web Station 需要此项）'
                        . '<br>③ SSH 执行：<code>chmod -R 755 ' . htmlspecialchars($newDataDir) . '</code>（必要时 777）';
                    $messageType = 'error';
                } else {
                    $detail = '目录不存在或无法读取。<br>' . implode('<br>', $diagDetails);
                    if (!empty($basedir)) {
                        $detail .= '<br>当前 PHP open_basedir 限制为：' . $basedir;
                    }
    ...
    // ── 运行数据目录（storage_dir）──
    // 所有运行期数据（config.json / app.db / session / cache / thumbs）统一存放于此。
    // 变更目录时 saveConfig() 会自动把现有数据搬到新位置。
    if (isset($_POST['storage_dir'])) {
        $newStorage = trim($_POST['storage_dir']);
        if ($newStorage === '') {
            // 留空 → 恢复默认：程序目录下的 runtime/
            $newStorage = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'runtime';
        }
        $newStorage = rtrim(normalizeConfigPath($newStorage), '/\\');
        if ($newStorage !== '') {
            clearstatcache(true);
            if (!is_dir($newStorage) && !@mkdir($newStorage, 0755, true)) {
                $message = '运行数据目录无效：无法创建该目录，请检查路径是否正确、PHP 进程是否有权限创建';
                $messageType = 'error';
            } elseif (!is_writable($newStorage)) {
                $message = '运行数据目录无效：目录不可写，请检查权限（PHP 进程需要对该目录有读写权限）';
                $messageType = 'error';
            } else {
                $config['storage_dir'] = $newStorage;
            }
        }
    }
    ...
    if ($message === '') {
        if (saveConfig($config) === false) {
            $message = '保存失败：无法写入配置文件 config.json，请检查运行数据目录（' . htmlspecialchars(getStorageDir()) . '）的写入权限';
            $messageType = 'error';
        } else {
            $message = '设置已保存';
        }
...
<div class="content-box">
    ...
    <form method="post">
        ...
        <div class="form-item">
            <label for="data_dir">数据目录（文件展示根目录）</label>
        ...
        <div class="form-item">
            <label for="storage_dir">运行数据目录（可留空）</label>
            <input type="text" id="storage_dir" name="storage_dir" value="<?php echo htmlspecialchars($config['storage_dir'] ?? ''); ?>" placeholder="例如 /volume1/web/filelist_data（留空则使用默认）">
            <div style="background: #e8f5e9; color: #2e7d32; padding: 10px; border-radius: 8px; font-size: 12px; margin-top: 5px;">
                📦 运行期数据（配置文件 config.json、数据库 app.db、Session、缓存、缩略图）统一存放在此目录。<br>
                升级程序时<strong>只要保留这个目录，所有设置、账号和数据都不会丢失</strong>（配置不再散落在程序文件里被覆盖）。<br>
                默认在程序目录的 <code>runtime/</code> 下，直接留空即可；如需彻底隔离，可填写项目外的绝对路径（如 <code>/volume1/web/filelist_data</code>），保存后程序会自动把现有数据迁移过去。
            </div>
        </div>
        <div class="form-item">
