...
/**
 * 读取 JSON 文件并解码为数组。
 * 兼容 UTF-8 BOM：Windows 记事本等工具保存的 UTF-8 文件常带 BOM，
 * 而 json_decode() 对 BOM 敏感（会解析失败导致配置回退为默认值），这里统一剥离。
 *
 * @param string $path
...
/**
 * 运行期数据目录：config.json / app.db / session / cache / thumbs 统一存放于此。
 * 默认在项目根目录下的 runtime/，可在后台「系统设置 → 运行数据目录」中改到项目外
 * （例如 /volume1/web/filelist_data）。升级程序时只要保留该目录，所有配置与数据不丢失。
 *
 * @param string|null $force 传入非 null 时用于刷新静态缓存（saveConfig 变更 storage_dir 后使用）
 */
function getStorageDir($force = null) {
    static $dir = null;
    if ($force !== null) {
        $dir = $force;
        return $dir;
    ...
    if ($dir === null) {
        migrateRuntimeData(); // 首次访问前，先自动迁移旧版散落在项目根目录的运行期数据
        $default = __DIR__ . DIRECTORY_SEPARATOR . 'runtime';
        $dir = $default;
        // 从已生效的 config.json 读取 storage_dir（runtime/ 优先，兼容旧版根目录 config.json）
        foreach (array($default . DIRECTORY_SEPARATOR . 'config.json', __DIR__ . DIRECTORY_SEPARATOR . 'config.json') as $f) {
            if (is_file($f)) {
                $data = readJsonFile($f);
                if (is_array($data) && !empty($data['storage_dir']) && is_string($data['storage_dir'])) {
                    $dir = rtrim($data['storage_dir'], '/\\');
                    break;
                }
            }
        }
...
/**
 * 自动迁移：把旧版本散落在项目根目录的运行期数据
 * （config.json / app.db / session / cache / thumbs）收拢到 storage_dir 下。
 * 升级代码后首次访问自动完成，老配置与账号数据不丢失。幂等，可安全重复执行。
 */
function migrateRuntimeData() {
    static $migrated = false;
    $root = __DIR__;
    $default = $root . DIRECTORY_SEPARATOR . 'runtime';

    // 快速路径：根目录已无运行期数据，且默认 runtime/ 已建立 → 无需迁移
    if ($migrated
        || (!file_exists($root . DIRECTORY_SEPARATOR . 'config.json')
            && !file_exists($root . DIRECTORY_SEPARATOR . 'app.db')
            && is_dir($default))) {
        $migrated = true;
        return;
    }

    // 确定目标目录：优先读根目录旧 config.json 中已配置的 storage_dir，其次 runtime/config.json
    $target = $default;
    foreach (array($root . DIRECTORY_SEPARATOR . 'config.json', $default . DIRECTORY_SEPARATOR . 'config.json') as $f) {
        if (is_file($f)) {
            $data = readJsonFile($f);
            if (is_array($data) && !empty($data['storage_dir']) && is_string($data['storage_dir'])) {
                $target = rtrim($data['storage_dir'], '/\\');
                break;
            }
        }
    }
    ...
    // 仅从项目根目录迁入（避免把 runtime/ 内已归档的数据再次搬动）
    foreach (array('config.json', 'app.db', 'session', 'cache', 'thumbs') as $name) {
        $old = $root . DIRECTORY_SEPARATOR . $name;
        $new = $target . DIRECTORY_SEPARATOR . $name;
        if (file_exists($old) && !file_exists($new)) {
            @rename($old, $new);
        }
    }

    // 迁移后：修正 config.json 中仍指向旧位置的 thumb_dir / session_dir，并补写 storage_dir
    $cfgFile = $target . DIRECTORY_SEPARATOR . 'config.json';
    if (is_file($cfgFile)) {
        $data = readJsonFile($cfgFile);
        if (is_array($data)) {
            $changed = false;
            if (empty($data['storage_dir']) || !is_string($data['storage_dir'])) {
                $data['storage_dir'] = $target;
                $changed = true;
            }
            $map = array('thumb_dir' => 'thumbs', 'session_dir' => 'session');
            foreach ($map as $key => $sub) {
                $oldDefault = $root . DIRECTORY_SEPARATOR . $sub;
                if (isset($data[$key]) && rtrim($data[$key], '/\\') === $oldDefault) {
                    $data[$key] = $target . DIRECTORY_SEPARATOR . $sub;
                    $changed = true;
                }
            }
            if ($changed) {
                @file_put_contents($cfgFile, json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
            }
        }
    }
...
function getConfigJsonPath() {
    return getStorageDir() . DIRECTORY_SEPARATOR . 'config.json';
}
...
function loadConfig() {
    ...
    // 优先从 config.json 加载（readJsonFile 兼容带 UTF-8 BOM 的文件）
...
function normalizeConfigPaths($config) {
    ...
    $pathKeys = array('data_dir', 'thumb_dir', 'session_dir', 'storage_dir');
...
function getDefaultConfig() {
    // config.php 是默认配置的唯一来源（含版本号），避免多份默认值导致不同步
    $phpFile = __DIR__ . DIRECTORY_SEPARATOR . 'config.php';
    if (is_file($phpFile)) {
        $cfg = require $phpFile;
        if (is_array($cfg)) {
    ...
    // 兜底（config.php 缺失时），与 config.php 保持一致
    return array(
        'site_name' => '文件浏览器',
        'site_subtitle' => '轻量级文件目录浏览系统',
        'data_dir' => __DIR__ . DIRECTORY_SEPARATOR . 'data',
        'storage_dir' => __DIR__ . DIRECTORY_SEPARATOR . 'runtime',
        'thumb_dir' => __DIR__ . DIRECTORY_SEPARATOR . 'runtime' . DIRECTORY_SEPARATOR . 'thumbs',
        'session_dir' => __DIR__ . DIRECTORY_SEPARATOR . 'runtime' . DIRECTORY_SEPARATOR . 'session',
        'max_upload_size' => 0,
        'hidden_files' => array(),
        'app_version' => '2.10.0',
        'icon_scheme' => 'emoji',
...
function saveConfig($config) {
    ...
    // storage_dir 变更 → 把运行期数据（app.db/session/cache/thumbs）搬到新目录
    $oldStorage = getStorageDir();
    $newStorage = '';
    if (!empty($config['storage_dir'])) {
        $newStorage = rtrim(normalizeConfigPath($config['storage_dir']), '/\\');
        if ($newStorage !== '' && $newStorage !== $oldStorage) {
            if (!is_dir($newStorage)) {
                @mkdir($newStorage, 0755, true);
            }
            foreach (array('app.db', 'session', 'cache', 'thumbs') as $item) {
                $src = $oldStorage . DIRECTORY_SEPARATOR . $item;
                $dst = $newStorage . DIRECTORY_SEPARATOR . $item;
                if (file_exists($src) && !file_exists($dst)) {
                    @rename($src, $dst);
                }
            }
            // thumb_dir / session_dir 若仍指向旧 storage 子目录，同步改为新位置
            $map = array('thumb_dir' => 'thumbs', 'session_dir' => 'session');
            foreach ($map as $key => $sub) {
                $oldSub = $oldStorage . DIRECTORY_SEPARATOR . $sub;
                if (isset($config[$key]) && rtrim($config[$key], '/\\') === $oldSub) {
                    $config[$key] = $newStorage . DIRECTORY_SEPARATOR . $sub;
                }
            }
            getStorageDir($newStorage); // 刷新静态缓存，config.json 将写入新位置
        }
    }

    $configFile = getConfigJsonPath();
    $saved = @file_put_contents($configFile, json_encode($config, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));

    // storage_dir 变更且新配置已成功写入 → 清理旧位置的残留 config.json
    if ($saved !== false && $newStorage !== '' && $newStorage !== $oldStorage) {
        $oldCfg = $oldStorage . DIRECTORY_SEPARATOR . 'config.json';
        if ($oldCfg !== $configFile && file_exists($oldCfg)) {
            @unlink($oldCfg);
        }
    }

    // 返回是否写入成功，供调用方在保存失败时给出明确提示（如目录不可写等）
    return ($saved !== false);
}

function getCacheDir() {
    return getStorageDir() . DIRECTORY_SEPARATOR . 'cache';
}

function initDirectories() {
    migrateRuntimeData(); // 确保旧数据已收拢到 storage_dir，再读取各目录
    ...
    foreach (array($dataDir, $thumbDir, $sessionDir, $cacheDir, $storageDir) as $dir) {
...
function getAppDbPath() {
    return getStorageDir() . DIRECTORY_SEPARATOR . 'app.db';
}
...
function isHiddenFile($fileName, $relativePath = '') {
    ...
    if (empty($hiddenFiles) || !is_array($hiddenFiles)) {
        return false;
    }
    // Windows 文件系统不区分大小写：规则 desktop.ini 也应匹配实际文件名 Desktop.ini
    ...
    foreach ($hiddenFiles as $rule) {
        ...
        if (strpos($rule, '*') !== false) {
            // 通配符规则：先转义正则特殊字符，再恢复 * 为 .*，避免 . [ ] 等被当作正则语法
            $pattern = str_replace('\*', '.*', preg_quote($rule, '/'));
            if (preg_match('/^' . $pattern . '$/' . ($caseInsensitive ? 'i' : ''), $fileName)) {
                return true;
            }
        } else {
            if (substr($rule, -1) === '/' || substr($rule, -1) === '\\') {
                // 目录规则（folder/ 或 folder\）
                $dirName = rtrim($rule, '/\\');
                if ($caseInsensitive) {
                    if (strcasecmp($fileName, $dirName) === 0 || stripos($relativePath, $dirName . '/') === 0) {
                        return true;
                    }
                } else {
                ...
                // 文件/路径规则
                if ($caseInsensitive) {
                    if (strcasecmp($fileName, $rule) === 0 || stripos($relativePath, $rule) === 0) {
                        return true;
                    }
                } else {
