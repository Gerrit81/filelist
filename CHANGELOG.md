...
### 修复
- 🐛 修复后台「安全审计」页面 403 错误：`blockSensitiveFiles()` 误拦截 `admin/security.php`，现在只拦截根目录核心文件
...
### 修复
- 🐛 修复前台文件大小列显示不全（如 `674.97 MB` 被截断），增大列宽至 130px
- 🐛 修复前台文件类型列显示不全（如"PowerPoint 演示文稿"被截断），列宽增至 175px
