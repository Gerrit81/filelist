...
### 改进
- 🛡️ 后台「隐藏文件管理」新增防呆提示：规则内部含空格（如 `sync .ffs_db`，常见误输入）时，添加后以黄色警告提示，并在规则列表中标注「⚠ 含空格」，避免因空格导致规则永远无法匹配文件
...
### 样式调整
- 🎯 返回顶部按钮位置微调：相对文件列表右缘再向右移动约两个中文字符，位于列表与屏幕右缘之间（标准/窄版布局同步适配）
...
### 新增
- ⬆️ 前台新增「返回顶部」悬浮按钮：页面滚动超过一定距离后自动出现，点击平滑返回顶部；SVG 箭头图标随主题渐变配色，位置紧贴文件列表右侧边缘（自动适配标准/窄版两种布局），并避开复制提示气泡
...
### 修复
- 🐛 修复 Windows 下隐藏文件不生效：配置文件 `config.json` 若被记事本等工具保存为带 UTF-8 BOM 的编码，`json_decode` 会解析失败导致配置整体丢失回退默认值；现统一剥离 BOM，兼容此类文件
- 🐛 修复隐藏规则因**大小写不匹配**而失效：Windows 文件系统不区分大小写，此前规则 `desktop.ini` 无法匹配实际文件名 `Desktop.ini`（如 `Thumbs.db`、`Desktop.ini` 等系统文件），现 Windows 下隐藏规则匹配改为大小写不敏感
- 🐛 修复通配符隐藏规则（如 `*.tmp`）中正则特殊字符被误解释的问题：规则先转义再匹配，避免 `a.b*` 这类规则把 `.` 当任意字符
- 🛡️ 配置保存失败不再静默：`saveConfig()` 返回写入结果，后台隐藏文件管理、系统设置、导航链接编辑在无法写入 `config.json`（如运行数据目录无写权限）时给出明确错误提示，避免"以为保存成功、实际未生效"的困惑
...
### ✨ 架构整理：运行期数据与程序代码分离
- 📦 新增「运行数据目录」`storage_dir`（默认 `runtime/`，可在后台系统设置中改为项目外路径）：`config.json`、`app.db`、`session/`、`cache/`、`thumbs/` 统一存放，**升级程序时只要保留该目录，配置、账号与数据全部不丢**
- 🔁 自动迁移：升级后首次访问自动把旧版散落在根目录的 `config.json` / `app.db` / `session` / `cache` / `thumbs` 收拢到 `runtime/`，无需手动移动
- 🚚 变更 `storage_dir` 时自动搬家：后台修改运行数据目录后，数据库、Session、缓存、缩略图自动迁移到新位置
- 🏷️ 版本号唯一来源：`functions.php` 的默认配置改为直接读取 `config.php`，消除多份默认值不同步问题（此前前台版本号与后台更新日志不一致即源于此）
- 🧹 前台 LibreOffice 预览缓存目录改走统一的缓存目录
...
### 增强
- 🔧 数据目录验证新增**逐级路径诊断**：自动定位断链目录、检测权限不足层级、列出卷下实际共享文件夹名称，帮助快速定位「目录不存在或无法读取」的真正原因
- 💡 补充群晖 Web Station 高级共享权限（"对所有应用程序授予访问权限"）排查提示，并说明 open_basedir 选项位于 PHP 配置文件编辑页的「设置/核心设置」选项卡
...
### 修复
- 🐛 修复群晖 NAS WebStation 下设置数据目录误报「目录不存在或无法读取」：群晖默认禁用 `shell_exec`，原 open_basedir 的 shell 二次检测失效，现改为解析 `open_basedir` 配置做路径前缀匹配，直接识别拦截并给出 Web Station 专属修复指引；同时补充 http 用户目录权限排查提示
