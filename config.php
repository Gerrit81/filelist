...
return array(
    ...
    // ─── 运行期数据目录 ────────────────────────
    // config.json / app.db / session / cache / thumbs 统一存放在此目录下。
    // 升级程序时只要保留该目录，所有配置、账号与数据都不会丢。
    // 默认在项目根目录的 runtime/；建议部署后到后台「系统设置 → 运行数据目录」
    // 改成项目外路径（如 /volume1/web/filelist_data），这样升级时可彻底不受影响。
