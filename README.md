...
```
filelist/
├── runtime/             ← 运行期数据目录（自动创建）
│   ├── config.json      ← 运行期配置（后台修改的设置都存这里）
│   ├── app.db           ← SQLite 数据库（账号/权限/下载记录）
│   ├── session/         ← Session 存储
│   ├── cache/           ← 目录列表缓存
│   └── thumbs/          ← 图片缩略图缓存
...
> 💡 旧版本散落在程序根目录的 `config.json` / `app.db` / `session` / `cache` / `thumbs`
> 会在升级后首次访问时**自动迁移**到 `runtime/` 下，无需手动移动。
...
| `storage_dir` | 运行期数据目录（config.json / app.db / session / cache / thumbs），可填项目外路径 | 程序目录下的 `runtime/` |
...
├── functions.php         # 核心函数库
├── config.php            # 默认配置（只读默认值，版本号唯一来源）
├── security.php          # 安全防护（SQLite / CSRF / 路径校验）
├── favicon.svg           # 网站图标
...
├── data/                 # 文件存储目录（把要分享的文件放这里）
└── runtime/              # ★ 运行期数据目录（升级时保留，绝不覆盖）
    ├── config.json       # 运行期配置（后台设置）
    ├── app.db            # SQLite 数据库
    ├── session/          # Session
    ├── cache/            # 缓存
    └── thumbs/           # 缩略图
...
**程序代码与运行期数据已经分离**，升级时只需保留 `runtime/` 目录即可，配置、账号、数据都不丢：

1. 下载新版本 → 解压；
2. **把旧目录中的 `runtime/` 目录保留**（如有自定义过 `data_dir`，`data/` 或目标文件目录同样保留，它本来就在程序之外）；
3. 用新文件覆盖旧文件（**不要删除/覆盖 `runtime/`**）；
4. 完成后重新访问站点，旧配置与数据自动生效。

> 推荐做法：在后台「系统设置 → 运行数据目录」填一个**项目外的路径**（如群晖 `/volume1/web/filelist_data`），
> 保存后程序自动把数据迁移过去。此后升级程序目录时完全不受影响，`runtime/` 里只会留下一个指向该路径的索引文件。

**数据备份** = 备份 `runtime/` 目录（或你配置的项目外运行数据目录）即可，无需再逐个找配置文件。
